...
# read data
prefs = CSV.read(indir * "example-preferences.csv", DataFrames.DataFrame);
...
allocations = Dict();
for order in collect(permutations(1:length(individuals)))
    println("order: $order")
...
CSV.write(outdir * "serial-dictator-allocation.csv", DataFrames.DataFrame(optimal_allocation));
